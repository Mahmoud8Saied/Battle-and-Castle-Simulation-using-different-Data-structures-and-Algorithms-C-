...
void GUI :: Printinfo(string msg)
{
	pWind->SetPen(DARKRED);
	pWind->SetFont(12, BOLD , BY_NAME, "Arial");   
	pWind->DrawString(0, WindHeight - (int) (StatusBarHeight/2), msg);
}


//prints info on status bar. 
//Info includes timestep and other enemies info as illustrated in project doc
void GUI::UpdateStatusBar(int CurrentTimeStep, string msg)	
{
...
void GUI::UpdateInterface(int CurrentTimeStep, string msg) 
{
	ClearDrawingArea();
	UpdateStatusBar(CurrentTimeStep, msg);
	DrawCastleArea();
	DrawAllItems();
}
